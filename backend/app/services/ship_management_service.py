...
from app.schemas.invoice import ExternalHandoffCreate, InvoiceBulkAuditAssign
...
def list_batches(db: Session, from_date: date | None, to_date: date | None) -> list[dict]:
    today = datetime.now(VIETNAM_TIMEZONE).date()
    effective_from, effective_to = from_date or today, to_date or today
    if effective_from > effective_to:
        raise HTTPException(status_code=422, detail="Ngày bắt đầu không được sau ngày kết thúc")
    start_utc, _ = vietnam_day_utc_bounds(effective_from)
    _, end_utc = vietnam_day_utc_bounds(effective_to)
    batches = db.scalars(batch_query().where(
        ExternalHandoverBatch.handed_over_at >= start_utc,
        ExternalHandoverBatch.handed_over_at < end_utc,
    ).order_by(ExternalHandoverBatch.handed_over_at.desc(), ExternalHandoverBatch.id.desc())).unique().all()
