...
@router.get("", response_model=list[ProductCategoryRead], dependencies=[Depends(require_any_permission("product_categories.view", "products.view", "products.create", "products.update"))])
def list_categories(
...
@router.post("", response_model=ProductCategoryRead, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_permission("product_categories.create"))])
...
@router.get("/{category_id}", response_model=ProductCategoryRead, dependencies=[Depends(require_permission("product_categories.view"))])
def get_category(category_id: int, db: Session = Depends(get_db)):
    return product_category_service.get_category(db, category_id)


@router.put("/{category_id}", response_model=ProductCategoryRead, dependencies=[Depends(require_permission("product_categories.update"))])
...
@router.delete("/{category_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_permission("product_categories.delete"))])
